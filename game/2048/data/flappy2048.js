...
var cndSrc = "http://gtg.y3.cn/html5/single/2048/";
...
score.endUpdate = function() {
    for (var i = score.divs.length - 1; i >= 0; i--) {
		...
		score.divs[i].innerHTML = "得分：" + points + " | 最高：<span style='color:#F00;'>" + highscore + "</span>，正在努力查询您的排名...";
		var wxTitle = "比2048和Flappy Bird还要虐心的游戏我已经玩到"+highscore+"分啦！速度来挑战我！";
		...
		if(points > highscore){
			highscore = points;
			gameLocal.updateGameScore(highValue,gameLocal,function(rankValue){
				rank = rankValue;
				//alert("r - " + rankValue[0] + " | " + rankValue[1]);
				
				gameLocal.setBestScore(highscore);
				score.endUpdate();
			});
		}
		else{
			if(rank[0] == 0){
				gameLocal.getRank(highValue,gameLocal,function(rankValue){
					rank = rankValue;
					//alert("r - " + rankValue[0] + " | " + rankValue[1]);
				...
				score.divs[i].innerHTML = "得分:" + points + " | 最高:<span style='color:#F00;'>" + highscore + "</span><br />在"+rank[0]+"人中排名第<span style='color:#F00;'>" + rank[1] + "</span>！";
				var wxTitle = "比2048和Flappy Bird还要虐心的游戏我已经玩到"+highscore+"分啦！在"+rank[0]+"个人里已经排名第"+rank[1]+"！速度来挑战我！";
				setWxContent(wxTitle);
			}
		}
    }
...
setWxContent = function (title) {
	if(typeof(window.shareData) != "undefined" ){
		...
		window.shareData["wContent"] = title + " —— " + window.shareData["fContent"] + "【http://bbs.y3.cn/wechat/website/games/single/flappy2048/】";
	}
